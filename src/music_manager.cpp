...
std::string MusicManager::get_current_song() {
    return loaded_songs[current_playing_song].title;
}

void MusicManager::load_songs() {
    std::string path;
    for(size_t i = 1; i <= loaded_songs.size(); i++){
        path = "song/" + loaded_songs[i].title + ".mp3";
        std::cout << path << "\n";
...
void MusicManager::load() {
    ...
    for(auto& s : songs) {
        ...
        if(s.contains("id"))
            loaded_songs.emplace(s["id"].get<int>(), song);
...
void MusicManager::print_songs_data() {
    for(const auto& [key, value] : loaded_songs) {
